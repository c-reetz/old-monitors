...
func MensShoes(server string) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("Recovered in MensShoes", r)
		}
		for {
			mensShoesLauncher(server)
		}
	}()
}

func mensShoesLauncher(server string) {
	fmt.Println("Launching Ralph Lauren Mens Shoes Monitor")
	...
	//LOOP START
	for {
		err = mongoClient.Database("Monitors").Collection("Ralph Lauren").FindOne(context.TODO(), bson.M{"Subsection": "Mens Shoes"}).Decode(&previous)
		...
		req, err := http.NewRequest(http.MethodGet, "https://www.ralphlauren.eu/dk/en/men/shoes/1040?srule=price-low-high&start=0&sz=32&webcat=Men%7CShoes&format=ajax", nil)
		if err != nil {
			log.Println(err)
		...
		for _, product := range productList {
			...
			if price < 200 {
				...
				if !isInPreviousLoop {
					...
					sendPriceErrorRalphMensShoes(productName, productStock, productUrl, productImage, path[0], webhookSettings.Webhooks)
				}
			} else {
				continue
			}
		}
		_, err = mongoClient.Database("Monitors").Collection("Ralph Lauren").UpdateOne(context.TODO(), bson.M{"Subsection": "Mens Shoes"}, bson.M{"$set": bson.M{"Items": products}})
		if err != nil {
...
func sendPriceErrorRalphMensShoes(productName string, productStock string, productUrl string, productImg string, productPrice string, webhooks []string) {
	...
	_, err = webhook.SendMessage(&dishooks.WebhookMessage{
		Username: "Ralph Lauren Mens Shoes - Under 200 DKK",
