...
func WomensClothing(server string) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("Recovered in WomensClothing", r)
		}
		for {
			womensclothingLauncher(server)
		}
	}()
}

func womensclothingLauncher(server string) {
	fmt.Println("Launching Ralph Lauren Womens Clothing Monitor")
	...
	//LOOP START
	for {
		err = mongoClient.Database("Monitors").Collection("Ralph Lauren").FindOne(context.TODO(), bson.M{"Subsection": "Womens Clothing"}).Decode(&previous)
		...
		req, err := http.NewRequest(http.MethodGet, "https://www.ralphlauren.eu/dk/en/women/clothing/2020?srule=price-low-high&start=0&sz=32&webcat=Women%7CClothing&format=ajax", nil)
		if err != nil {
			log.Println(err)
		...
		for _, product := range productList {
			...
			if price < 200 {
				...
				if !isInPreviousLoop {
					...
					sendPriceErrorRalphWomensClothing(productName, productStock, productUrl, productImage, path[0], webhookSettings.Webhooks)
				}
			} else {
				continue
			}
		}
		_, err = mongoClient.Database("Monitors").Collection("Ralph Lauren").UpdateOne(context.TODO(), bson.M{"Subsection": "Womens Clothing"}, bson.M{"$set": bson.M{"Items": products}})
		if err != nil {
...
func sendPriceErrorRalphWomensClothing(productName string, productStock string, productUrl string, productImg string, productPrice string, webhooks []string) {
	...
	embed := dishooks.Embed{
		...
		Author: &dishooks.EmbedAuthor{
			Name: "Ralph Lauren Womens Clothing",
	...
	_, err = webhook.SendMessage(&dishooks.WebhookMessage{
		Username: "Ralph Lauren Womens Clothing - Under 200 DKK",
