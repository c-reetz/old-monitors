...
func WomensShoes(server string) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("Recovered in MensShoes", r)
		}
		for {
			womensShoesLauncher(server)
		}
	}()
}

func womensShoesLauncher(server string) {
	fmt.Println("Launching Ralph Lauren Womens Shoes Monitor")
	...
	//LOOP START
	for {
		err = mongoClient.Database("Monitors").Collection("Ralph Lauren").FindOne(context.TODO(), bson.M{"Subsection": "Womens Shoes"}).Decode(&previous)
		...
		req, err := http.NewRequest(http.MethodGet, "https://www.ralphlauren.eu/dk/en/women/shoes/2040?srule=price-low-high&start=0&sz=32&webcat=Women%7CShoes&format=ajax", nil)
		if err != nil {
			log.Println(err)
		...
		for _, product := range productList {
			...
			if price < 200 {
				...
				if !isInPreviousLoop {
					...
					sendPriceErrorRalphWomensShoes(productName, productStock, productUrl, productImage, path[0], webhookSettings.Webhooks)
				}
			} else {
				continue
			}
		}
		_, err = mongoClient.Database("Monitors").Collection("Ralph Lauren").UpdateOne(context.TODO(), bson.M{"Subsection": "Womens Shoes"}, bson.M{"$set": bson.M{"Items": products}})
		if err != nil {
...
func sendPriceErrorRalphWomensShoes(productName string, productStock string, productUrl string, productImg string, productPrice string, webhooks []string) {
	...
	embed := dishooks.Embed{
		...
		Author: &dishooks.EmbedAuthor{
			Name: "Ralph Lauren Womens Shoes",
	...
	_, err = webhook.SendMessage(&dishooks.WebhookMessage{
		Username: "Ralph Lauren Womens Shoes - Under 200 DKK",
