...
func WomensAccessory(server string) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("Recovered in WomensAccessory", r)
		}
		for {
			womensaccessoryLauncher(server)
		}
	}()
}

func womensaccessoryLauncher(server string) {
	fmt.Println("Launching Ralph Lauren Womens Accessory Monitor")
	...
	//LOOP START
	for {
		err = mongoClient.Database("Monitors").Collection("Ralph Lauren").FindOne(context.TODO(), bson.M{"Subsection": "Womens Accessory"}).Decode(&previous)
		...
		req, err := http.NewRequest(http.MethodGet, "https://www.ralphlauren.eu/dk/en/women/accessories/2030?srule=price-low-high&start=0&sz=32&webcat=Women%7CAccessories&format=ajax", nil)
		if err != nil {
			log.Println(err)
		...
		for _, product := range productList {
			...
			if price < 200 {
				...
				if !isInPreviousLoop {
					...
					sendPriceErrorRalphWomensAccessory(productName, productStock, productUrl, productImage, path[0], webhookSettings.Webhooks)
				}
			} else {
				continue
			}
		}
		_, err = mongoClient.Database("Monitors").Collection("Ralph Lauren").UpdateOne(context.TODO(), bson.M{"Subsection": "Womens Accessory"}, bson.M{"$set": bson.M{"Items": products}})
		if err != nil {
...
func sendPriceErrorRalphWomensAccessory(productName string, productStock string, productUrl string, productImg string, productPrice string, webhooks []string) {
	...
	embed := dishooks.Embed{
		...
		Author: &dishooks.EmbedAuthor{
			Name: "Ralph Lauren Womens Accessory",
			URL:  "https://www.ralphlauren.eu/dk/en/women/accessories/2030?srule=price-low-high&start=0&sz=32&webcat=Women%7CClothing",
		},
	...
	_, err = webhook.SendMessage(&dishooks.WebhookMessage{
		Username: "Ralph Lauren Womens Accessory - Under 200 DKK",
