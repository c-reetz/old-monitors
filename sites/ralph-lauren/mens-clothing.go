...
func MensClothing(server string) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("Recovered in MensClothing", r)
		}
		for {
			mensClothingLauncher(server)
		}
	}()
}

func mensClothingLauncher(server string) {
	fmt.Println("Launching Ralph Lauren Mens Clothing Monitor")
	...
	//LOOP START
	for {
		err = mongoClient.Database("Monitors").Collection("Ralph Lauren").FindOne(context.TODO(), bson.M{"Subsection": "Mens Clothing"}).Decode(&previous)
		...
		req, err := http.NewRequest(http.MethodGet, "https://www.ralphlauren.eu/dk/en/men/clothing/1020?srule=price-low-high&start=0&sz=32&webcat=Men%7CClothing&format=ajax", nil)
		if err != nil {
			log.Println(err)
		...
		for _, product := range productList {
			...
			if price < 200 {
				...
				if !isInPreviousLoop {
					...
					sendPriceErrorRalphMensClothing(productName, productStock, productUrl, productImage, path[0], webhookSettings.Webhooks)
				}
			} else {
				continue
			}
		}
		_, err = mongoClient.Database("Monitors").Collection("Ralph Lauren").UpdateOne(context.TODO(), bson.M{"Subsection": "Mens Clothing"}, bson.M{"$set": bson.M{"Items": products}})
		if err != nil {
...
func sendPriceErrorRalphMensClothing(productName string, productStock string, productUrl string, productImg string, productPrice string, webhooks []string) {
	...
	embed := dishooks.Embed{
		...
		Author: &dishooks.EmbedAuthor{
			Name: "Ralph Lauren Mens Clothing",
			URL:  "https://www.ralphlauren.eu/dk/en/men/clothing/",
		},
	...
	_, err = webhook.SendMessage(&dishooks.WebhookMessage{
		Username: "Ralph Lauren Mens Clothing - Under 200 DKK",
